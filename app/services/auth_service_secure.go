...
// Returns minimal JWT token + user context for authorization
func (s *SecureAuthService) SecureLogin(ctx context.Context, username, password string) (string, interface{}, error) {
	...
	if err == nil {
		...
		// Generate secure JWT token (minimal payload)
		token, loginCtx, err := s.generateSecureToken(authData)
		if err != nil {
			return "", nil, err
		}
...
func (s *SecureAuthService) databaseSecureLogin(ctx context.Context, username, password string) (string, interface{}, error) {
	...
	// Generate secure token and context
	token, loginCtx, err := s.generateSecureToken(authData)
	if err != nil {
		return "", nil, err
	}
