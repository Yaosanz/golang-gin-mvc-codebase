...
func configureConnectionPool(db *gorm.DB, cfg *config.Config) error {
	...
	if cfg.Database().MaxOpenConnection != "" {
		maxOpenConns, err := strconv.Atoi(cfg.Database().MaxOpenConnection)
		if err == nil && maxOpenConns > 0 {
			poolCfg.MaxOpenConns = maxOpenConns
		}
	}

	if cfg.Database().MaxConnectionLifetime != "" {
		if lifetime, err := time.ParseDuration(cfg.Database().MaxConnectionLifetime); err == nil {
			poolCfg.ConnMaxLifetime = lifetime
		}
	}

	if cfg.Database().MaxIdleLifetime != "" {
		if idleTime, err := time.ParseDuration(cfg.Database().MaxIdleLifetime); err == nil {
...
func verifyConnectionPool(sqlDB *sql.DB, timeout time.Duration) error {
	...
	stats := sqlDB.Stats()
	log.Printf("Connection pool configured successfully. MaxOpenConns: %d InUse: %d, Idle: %d",
